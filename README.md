...
| 引脚 | 功能     | 连接设备                |
| :--- | :------- | :---------------------- |
| PA8  | PWM      | 左前电机 PWM (ENA)      |
| PB12 | 数字输出 | 左前电机 IN1            |
| PB13 | 数字输出 | 左前电机 IN2            |
| PA9  | PWM      | 右前电机 PWM (ENB)      |
| PB14 | 数字输出 | 右前电机 IN3            |
| PB15 | 数字输出 | 右前电机 IN4            |
| PA10 | PWM      | 左后电机 PWM (ENA)      |
| PB0  | 数字输出 | 左后电机 IN1            |
| PB1  | 数字输出 | 左后电机 IN2            |
| PB3 | PWM      | 右后电机 PWM (ENB)      |
| PB2 | 数字输出 | 右后电机 IN3            |
| PA12 | 数字输出 | 右后电机 IN4            |
| PA0  | PWM      | 云台俯仰舵机            |
| PA1  | PWM      | 云台水平舵机            |
...
|                |      | 水弹波箱     |
| DC-DC降压模块1 | 5V   | 所有舵机     |
| DC-DC降压模块2 | 3.3V | STM32核心板  |
|                |      |              |
#### 电机驱动接线详情

```
/* 
L298N/TB6612电机驱动连接说明：
...
云台系统：
1. 俯仰舵机 (上下) -> PA0 (TIM2_CH1)
2. 偏航舵机 (左右) -> PA1 (TIM2_CH2)

机械臂系统：
3. 升降舵机 (MG996R) -> PB10 (TIM2_CH3)
   - 棕色线 -> GND
   - 红色线 -> 5V
   - 橙色线 -> 信号线
4. 夹爪舵机 (SG90) -> PB11 (TIM2_CH4)

注意：舵机需要5V供电，请确保DC-DC降压模块输出足够
*/
```
...
int main(void) {
    ...
    while (1) {
        ...
        if(HAL_GetTick() - led_tick > 500) {
            HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_3); // LED闪烁
...
void Check_Battery_Voltage(void) {
    #define BATTERY_MIN_VOLTAGE 10.0f  // 12V电池最低电压
    
    float voltage = ADC_GetBatteryVoltage();
    if(voltage < BATTERY_MIN_VOLTAGE) {
        // 低电量警告
        HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_3); // 快速闪烁
        // 限制电机功率
        Motion_SetMaxSpeed(128); // 降低最大速度
    }
}
```
...
- **慢速稳定**：不开环控制下，快速移动容易失控
- **分段移动**：分小段移动，每段结束后微调位置
- **操作手经验**：靠视觉判断距离和位置
...
void Motion_ControlWithPS2(Motion_TypeDef* motion, uint8_t LJoy_UD, uint8_t LJoy_LR, uint8_t btn1, uint8_t btn2)
{
    ...
    // 在Motion_ControlWithPS2函数中保持L1/R1转向控制
    if(btn2 & 0x01) { // L1 - 原地左转
        Motion_Move(motion, MODE_SPIN_LEFT, 150);
        special_mode = 1;
    } else if(btn2 & 0x02) { // R1 - 原地右转
...
- **示教编程**：赛前录制关键位置，比赛时一键调用
- **自适应抓取**：根据传感器反馈调整抓取力度
- **防撞保护**：机械臂运动过程中的碰撞检测
- **物资计数**：通过传感器统计成功收集的物资数量
